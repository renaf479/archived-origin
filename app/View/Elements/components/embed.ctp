	...
	<script type="text/javascript">
		var componentCtrl = function($scope, $rootScope) {
			...
			$rootScope.creatorModalSave = function() {				
				$scope.editor.render	= ($scope.editor.content.iframe)? '<iframe class="embed" src="http://<?php echo $_SERVER['HTTP_HOST'];?>/adIframe/%model%/%id%" frameborder="0" scrolling="no" collapse="iframe"></iframe>': '<div class="embed" collapse="content">'+$scope.editor.content.embed+'</div>';
